...
- Install application payload under `%ProgramFiles%\FrameRelay`. Register the MSI with Windows Installer/ARP, provide a Start Menu shortcut, and set uninstall metadata. Use WiX major-upgrade handling so a newer release upgrades the same product and older releases cannot replace newer versions. Since Windows Installer compares only the first three numeric version fields, allow equal numeric versions to replace each other so successive prereleases with the same core version do not install side by side. Same-core artifacts are not ordered by the MSI; either can replace the other.
...
3. A higher-version or equal-numeric-version major upgrade retains one installed-app entry and replaces the prior installation; uninstall removes installed app files and leaves user data/logs intact.
